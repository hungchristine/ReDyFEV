...
def fig1_generator(axis_type, size_ind, marker_clr, net_trade=None, xlim=None, ylim=None):
    ...
    plt.fill_between([0, ax.get_xlim()[1]], [0, ax.get_xlim()[1] * 1.1], [0, ax.get_xlim()[1] * 0.9], color="grey", alpha=0.13)
    plt.fill_between([0, ax.get_xlim()[1]], [0, ax.get_xlim()[1] * 1.2], [0, ax.get_xlim()[1] * 0.8], color="grey", alpha=0.1)
    plt.fill_between([0, ax.get_xlim()[1]], [0, ax.get_xlim()[1] * 1.5], [0, ax.get_xlim()[1] * 0.5], color='grey', alpha=0.07)
    ...
    plt.fill_between([0, ax2.get_xlim()[1]], [0, ax2.get_xlim()[1]*1.1], [0, ax2.get_xlim()[1]*0.9], color="grey", alpha=0.13)
    plt.fill_between([0, ax2.get_xlim()[1]], [0, ax2.get_xlim()[1]*1.2], [0, ax2.get_xlim()[1]*0.8], color="grey", alpha=0.1)
    plt.fill_between([0, ax2.get_xlim()[1]], [0, ax2.get_xlim()[1]*1.5], [0, ax2.get_xlim()[1]*0.5], color='grey', alpha=0.07)
    ...
    for (country, country_data) in CFEL_sorted.iterrows():
        ...
        if country_data["Production mix intensity"] <= 100 and country_data["Consumption mix intensity"]  <= 100: #Adjust for countries in bottom left
            if country == "SE":
...
if include_TD_losses:
    fig1_generator('linear', 'consumption', mark_color, net_trade_TL, xlim=500, ylim=500)
else:
    fig1_generator('linear', 'consumption', mark_color, net_trade, xlim=500, ylim=500)



# %%

# Extra figure; plot import, exports and net trade f"or each country
"""
...
ax = plot_trades.iloc[:,0:2].plot(kind='bar', color=['xkcd:cadet blue','k'], stacked=True, width=1, rot=45, figsize=(20,8), grid=True, use_index=True)
...
### Figure 5 (histogram with fleet size vs ICEV:BEV ratio); prepare data

# Prepare the data
y_axis = 1 / results['RATIO BEV:ICEV', 'C', 'Consumption mix']
...
# ### Plot Figure 4 (differences for domestic production)

A_diff = (results['BEV footprint, EUR production', 'A', 'Consumption mix'] -
          results['BEV footprint', 'A', 'Consumption mix']) / results['BEV footprint', 'A', 'Consumption mix']
F_diff = (results['BEV footprint, EUR production', 'F', 'Consumption mix'] -
          results['BEV footprint', 'F', 'Consumption mix']) / results['BEV footprint', 'F', 'Consumption mix']
...
ax = sorted_fig_data.iloc[::-1].plot(kind='barh', figsize=(16, 12), position=0.5, stacked=True, cmap=fig4_cmap, width=0.95)
...
def annotate_map(ax, countries, values, cmap_range, threshold=0.8, round_dig=1, fontsize=7.5):
...
annotate_map(ax[0, 0], mapping_data[mapping_data['BEV footprint - Segment A - Production mix'].notna()].index.to_list(),
...
annotate_map(ax[1, 0], mapping_data[mapping_data['BEV footprint - Segment A - Consumption mix'].notna()].index.to_list(),
...
annotate_map(ax[0, 1], mapping_data[mapping_data['BEV footprint - Segment F - Production mix'].notna()].index.to_list(),
...
annotate_map(ax[1, 1], mapping_data[mapping_data['BEV footprint - Segment F - Consumption mix'].notna()].index.to_list(),
...
annotate_map(ax[0, 0],
             mapping_data[mapping_data['abs_diff_A'].notna()].index.to_list(),
             mapping_data[mapping_data['abs_diff_A'].notna()].abs_diff_A.values,
             max(rmax, np.abs(rmin)), threshold=threshold)
annotate_map(ax[0, 1],
             mapping_data[mapping_data['abs_diff_C'].notna()].index.to_list(),
             mapping_data[mapping_data['abs_diff_C'].notna()].abs_diff_C.values,
             max(rmax, np.abs(rmin)), threshold=threshold)
annotate_map(ax[1, 0],
             mapping_data[mapping_data['abs_diff_D'].notna()].index.to_list(),
             mapping_data[mapping_data['abs_diff_D'].notna()].abs_diff_D.values,
             max(rmax, np.abs(rmin)), threshold=threshold)
annotate_map(ax[1, 1],
             mapping_data[mapping_data['abs_diff_F'].notna()].index.to_list(),
