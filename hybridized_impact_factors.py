...
# For making human-readable indices
labels = Analysis.PRO_f

# Make DataFrame with human-readable indices for whole ecoinvent requirements matrix
...
set(Analysis.PRO_f.loc[Analysis.PRO_f['activityName'].str.contains('electricity')]['activityName'])
...
market_mixes = Analysis.PRO_f.loc[Analysis.PRO_f['activityName'].str.contains('electricity, high voltage, production mix')] # production mixes, do not include solar or waste (or Swiss pumped hydro)
waste_mixes = Analysis.PRO_f.loc[Analysis.PRO_f['activityName'].str.contains('electricity, from municipal waste incineration to generic market for electricity, medium voltage')] # production mixes, do not include solar
solar_mixes = Analysis.PRO_f.loc[Analysis.PRO_f['activityName'].str.contains('electricity, low voltage')] # use low voltage mixes to get shares of solar photovoltaic
...
#ecoinvent_tecs = list(set(df7.index.get_level_values(level=1)))
matches = []
match_keys = []
num_matches = dict((keyword,0) for keyword in tec_list)
#temp_tecdict = {v:k for k,v in tec_dict.items() if type(v) != list}
...
#entso_e_production = pd.read_excel('entsoe_export.xlsx',sheet_name='generation',header=0,index_col=[0])
entso_e_production = pd.read_csv('ENTSO_production_volumes.csv', header=0, index_col=[0])
...
lv_mix_ef = D_labels.loc[trade_only_mixes_lv]
...
lv_mix_ef_comp = D_labels_comp[trade_only_mixes_lv]
...
# Calculate tec means as quick check
...
with open('trade_final.pkl', 'rb') as handle:
